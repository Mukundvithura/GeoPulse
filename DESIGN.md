...
- **Italic Note** (400, 14.5px/1.6, italic, left ink rule at 13px indent): Derived and deferred states -- pending lines, and "surveyed but blank".
...
**The Italic Is A Convention Rule.** Spectral italic is not emphasis. It marks the hydrographic category of things that are submerged, derived, or deferred: the extracted action verb in a reported relation, pending and "surveyed but blank" notes, and the search field's placeholder. Never italicize for stress.

**The Literal String Rule.** A machine identifier is a literal string and is never recased or tracked out. Nothing in the interface renders one today; if one ever lands inside a Label, exempt it from the label's uppercase and 0.19em tracking rather than deforming it.
